    ...
    for asset in canonical_assets:
        ...
        if raw_columns:
            for col in raw_columns:
                ...
                is_col_sensitive = (
                    "ssn" in col_name.lower()
                    or "tax" in col_name.lower()
                    or "routing" in col_name.lower()
                    or "phone" in col_name.lower()
                    or "email" in col_name.lower()
                    or "salary" in col_name.lower()
                )
                ...
                # 4. Accuracy for Numeric Limits & Financials
                    ...
                    "limit" in col_name.lower()
                    or "salary" in col_name.lower()
                    or "amount" in col_name.lower()
                    or "balance" in col_name.lower()
                ):
                    rows.append(
                        {
                            "Asset_ID": asset_id,
...
def generate_tag_recommendations_csv(platform, raw_assets, canonical_assets, csv_output_file):
    ...
    for asset in canonical_assets:
        ...
        is_table_sensitive = any(
            c.lower() in ["pii", "phi", "pci", "confidential", "restricted", "sensitive"] for c in asset.classifications
        )
...
def build_pdf_report(platform, input_file, output_file):
    ...
    # Build Document
    doc = SimpleDocTemplate(output_file, pagesize=letter, leftMargin=54, rightMargin=54, topMargin=54, bottomMargin=54)
    ...
    th_style = ParagraphStyle(
        "TableHeader", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=9, leading=12, textColor=colors.white
    )
    ...
    td_indented_style = ParagraphStyle("TableCellIndented", parent=td_style, leftIndent=12)
    ...
    story.append(
        Paragraph(
            f"Platform: <b>{platform.upper()}</b> | Scope: Enterprise Implementation | Evaluation Date: {datetime.now().strftime('%Y-%m-%d')}",
            subtitle_style,
        )
    )
    ...
    summary_table = Table(
        [[Paragraph(summary_text, ParagraphStyle("SummaryText", parent=body_style, textColor=colors.HexColor("#2C5282")))]],
        colWidths=[504],
    )
    ...
    story.append(
        Paragraph(
            "Scores are computed across critical governance disciplines mapped directly from catalog metadata using transparent, auditable rules. Health indicators are nested under their respective disciplines:",
            body_style,
        )
    )
    ...
    for disp_key, disp_data in maturity_results["disciplines"].items():
        ...
        for ind_key, ind_data in disp_data["indicators"].items():
            ...
            if desc:
                consolidated_dashboard_data.append([Paragraph(desc, td_def_style), "", "", ""])
    ...
    tcs_banner_table = Table([[Paragraph(fin_section_subtitle, tcs_banner_style)]], colWidths=[504])
    tcs_banner_table.setStyle(
        TableStyle(
            [
    ...
    if tcs_result.tier == "low":
        ...
        suspended_table = Table(
            [
                [
                    Paragraph(
                        suspended_text, ParagraphStyle("Suspended", parent=body_style, textColor=colors.HexColor("#822727"))
                    )
                ]
            ],
            colWidths=[504],
        )
        suspended_table.setStyle(
            TableStyle(
                [
        ...
        financial_data = [
            [
                Paragraph("Financial Metric Category", th_style),
                Paragraph("Amount ($)", th_style),
                Paragraph("Value Explanation", th_style),
            ],
            [
            ...
            warning_table = Table(
                [
                    [
                        Paragraph(
                            warning_text,
                            ParagraphStyle("WarningNote", parent=body_style, textColor=colors.HexColor("#744210")),
                        )
                    ]
                ],
                colWidths=[504],
    ...
    if not vulnerabilities:
        story.append(
            Paragraph("No critical vulnerabilities detected. All assets comply with baseline policy checks.", body_style)
        )
    else:
        v_headers = [
    ...
    for idx, (disp_key, disp_info) in enumerate(discipline_details.items(), 1):
        ...
        sg_data = [
            [Paragraph("STRENGTHS", col_header_style), Paragraph("KEY GAPS / RISKS", col_header_style)],
            [
        ...
        story.append(KeepTogether([t_header, Spacer(1, 4), p_reasoning_header, Spacer(1, 2), p_reasoning_body, Spacer(1, 6)]))

        story.append(t_sg)
        story.append(Spacer(1, 4))

        # Keep actions header and actions text together
        story.append(KeepTogether([p_actions_header, Spacer(1, 2), p_actions_body, Spacer(1, 12)]))
    ...
    story.append(
        Paragraph(
            "<b>Model Baseline Constants & Citation Sources</b>",
            ParagraphStyle("SubheadingAppendix", parent=body_style, fontName="Helvetica-Bold", spaceAfter=2),
        )
    )
    params_data = [
        [
            Paragraph("Parameter Constants", th_style),
            Paragraph("Value", th_style),
            Paragraph("Research Context / Citation Source", th_style),
        ],
        [
            Paragraph(f"Total Cost of Ownership (TCO) - {platform.upper()}", td_style),
            Paragraph(f"{format_currency(operating_cost)}/yr", td_style),
            Paragraph(f"Estimated annual operating cost (licensing + administration headcount support).", td_style),
        ],
        [
            Paragraph("Loaded Analyst Rate", td_style),
            Paragraph(f"${roi_engine.hourly_analyst_rate:.2f}/hr", td_style),
            Paragraph("U.S. Labor Bureau senior loaded rate averages (1.35x multiplier).", td_style),
        ],
        [
            Paragraph("Hours Saved per Search", td_style),
            Paragraph(f"{roi_engine.hours_saved_per_search:.1f} hrs", td_style),
            Paragraph("Forrester TEI data catalog productivity metrics.", td_style),
        ],
        [
            Paragraph("Search/Discovery Ratio", td_style),
            Paragraph(f"{roi_engine.search_ratio * 100:.1f}%", td_style),
            Paragraph("Manual human query proportion versus scheduled pipeline queries.", td_style),
        ],
        [
            Paragraph("Annual Storage Cost / GB", td_style),
            Paragraph(f"${roi_engine.storage_cost_per_gb_year:.2f}", td_style),
            Paragraph("Standard hot cloud object tier blended averages (AWS, Azure).", td_style),
        ],
        [
            Paragraph("Data Quality Error Cost", td_style),
            Paragraph(f"${roi_engine.cost_per_dq_incident:,.2f}", td_style),
            Paragraph("Gartner & Monte Carlo operational cost studies.", td_style),
        ],
        [
            Paragraph("Compliance Breach Penalty", td_style),
            Paragraph(f"${roi_engine.cost_per_data_breach:,.2f}", td_style),
            Paragraph("IBM Cost of Data Breach localized/tier-1 infraction fine.", td_style),
        ],
    ]
    ...
    story.append(
        Paragraph(
            "<b>Equations & Derivations</b>",
            ParagraphStyle("SubheadingAppendix2", parent=body_style, fontName="Helvetica-Bold", spaceAfter=2),
        )
    )
    story.append(
        Paragraph(
            "1. <b>Productivity Savings</b>: (Annual Queries * 0.05% search ratio) * 3.5 hrs saved * Analyst loaded rate * (Doc Score / 100). Grounded in Forrester's TEI frameworks.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "2. <b>Redundant, Obsolete, and Trivial (ROT) Decommissioning</b>: Identified as size > 0, queries < 5/mo, and last accessed > 180 days. Savings = GB Size * $0.24/yr. Grounded in AWS/Azure storage tiers.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "3. <b>Data Quality (DQ) Incident Avoidance</b>: (Unmonitored baseline [5.0% probability] - Current probability) * $15k cost. Active Data Quality (DQ) profiling drops current probability to 2.0% (for DQ >= 80%) or 0.0% (for DQ >= 95%). Grounded in Gartner data quality impact surveys.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "4. <b>Risk Avoidance</b>: (Baseline breach prob. [5.0%] - Current prob.) * $150k breach penalty. Controls (ownership and classification) reduce probability to 1.0% (0.2% with active Data Quality (DQ) checks). Grounded in IBM Security breach statistics and DAMA DMBOK principles.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "5. <b>Compute Optimization</b>: Snowflake credit waste avoided on bad/unoptimized queries, calculated based on the active warehouse sizes, average credit rates ($3.00/credit), and active Data Quality (DQ) monitoring rules.",
            bullet_style,
        )
    )
    story.append(
        Paragraph("6. <b>Net Program Value</b>: Net Value = Total Realized Savings - Annual Operating Cost.", bullet_style)
    )
    story.append(
        Paragraph(
            "7. <b>Return on Investment (ROI) %</b>: ROI % = (Net Realized Program Value / Annual Operating Cost) * 100.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "8. <b>Unrealized Opportunity Value</b>: Sum of remaining discovery opportunity + Data Quality (DQ) opportunity + Risk mitigation + Redundant, Obsolete, and Trivial (ROT) storage decommissioning savings.",
            bullet_style,
        )
    )
    story.append(Spacer(1, 10))
    ...
    story.append(
        Paragraph(
            "<b>Discipline Maturity & Indicator Assessment Formulas</b>",
            ParagraphStyle("SubheadingAppendix3", parent=body_style, fontName="Helvetica-Bold", spaceAfter=2),
        )
    )
    story.append(
        Paragraph(
            "• <b>Documentation Coverage %</b>: (Assets with description >= 50 characters / Total Assets) * 100. Thresholds: [40, 60, 75, 90]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Ownership Assignment %</b>: (Assets with >= 1 owner / Total Assets) * 100. Thresholds: [40, 60, 75, 90]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Glossary Linkage %</b>: (Assets with >= 1 glossary term / Total Assets) * 100. Thresholds: [40, 60, 75, 90]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Classification Coverage %</b>: (Assets with >= 1 classification / Total Assets) * 100. Thresholds: [40, 60, 75, 90]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Data Quality (DQ) Rule Coverage %</b>: (Assets with >= 1 DQ rules run / Total Assets) * 100. Thresholds: [25, 50, 70, 85]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Data Quality (DQ) Pass Rate %</b>: (Total rules passed / Total rules run) * 100. Thresholds: [70, 80, 90, 95]%.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Maturity Scoring Step-mapping</b>: Raw percentages map to 1-5 levels based on the thresholds: Score = 1.0 (if &lt; thresholds[0]), 2.0 (if &lt; thresholds[1]), 3.0 (if &lt; thresholds[2]), 4.0 (if &lt; thresholds[3]), 5.0 (if &gt;= thresholds[3]).",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Metadata Management Maturity</b>: Weighted average of its indicators: Documentation (30%), Ownership (30%), Glossary Linkage (20%), and Classification (20%).",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Data Quality Maturity</b>: Weighted average of its indicators: Rule Coverage (40%) and Pass Rate (60%).",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Overall Maturity Index</b>: Weighted rollup of Metadata Management (50%) and Data Quality (50%).",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Telemetry Confidence Score (TCS)</b>: Blended average of size, query, owner, and lineage coverages: TCS % = (Size Coverage + Query Coverage + Owner Coverage + Lineage Coverage) / 4.",
            bullet_style,
        )
    )
    story.append(
        Paragraph("• <b>Size Coverage %</b>: (Assets with storage size &gt; 0 bytes / Total Assets) * 100.", bullet_style)
    )
    story.append(Paragraph("• <b>Query Coverage %</b>: (Assets with query count &gt; 0 / Total Assets) * 100.", bullet_style))
    story.append(
        Paragraph(
            "• <b>Owner Coverage %</b>: (Assets with technical or business ownership &ge; 1 / Total Assets) * 100.",
            bullet_style,
        )
    )
    story.append(
        Paragraph(
            "• <b>Lineage Coverage %</b>: (Assets with mapped upstream or downstream lineage relations / Total Assets) * 100.",
            bullet_style,
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="alation/sample_alation_metadata.json",
        help="Input JSON file containing platform mock metadata",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output PDF report path. If omitted, saves dynamically in a new run folder inside 'reports/'.",
    )
