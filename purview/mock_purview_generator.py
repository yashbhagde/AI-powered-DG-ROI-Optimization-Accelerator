...
def generate_purview_metadata(num_assets=None):
    ...
    purview_export = [
        # 1. PowerBI Dashboard Asset: executive_financial_dashboard
        {
            ...
            "classifications": [{"typeName": "Financial.Restricted", "validity": "CONFIRMED"}],
            "meanings": [
            ...
            "usage": {"queries": 1200, "users": 45, "lastAccessed": (now - timedelta(hours=1)).isoformat() + "Z"},
        },
        # 2. ROT Table Asset: archive_backup_database_2020
        {
            ...
            "usage": {"queries": 0, "users": 0, "lastAccessed": (now - timedelta(days=700)).isoformat() + "Z"},
        },
        # 3. Azure SQL Table Asset: vendor_billing_info
        {
            ...
            "contacts": {"Owner": [{"id": "ap_stewards", "info": "accounts_payable@company.com"}]},
            "classifications": [
                {"typeName": "MICROSOFT.FINANCIAL.CREDIT_CARD_NUMBER", "validity": "CONFIRMED"},
                {"typeName": "PII", "validity": "CONFIRMED"},
            ],
            "meanings": [{"displayText": "Vendor Account", "termGuid": "t_vendor_acc"}],
            "dataQuality": {
                "rulesRun": 15,
            ...
            "usage": {"queries": 450, "users": 12, "lastAccessed": (now - timedelta(hours=3)).isoformat() + "Z"},
        ...
        # 4. Schema Asset
        {
            ...
            "lineage": {"inputs": [], "outputs": ["purview_5005"]},
            "usage": {"queries": 150, "users": 4, "lastAccessed": (now - timedelta(hours=6)).isoformat() + "Z"},
        },
        # 5. View Asset
        {
            "guid": "5005",
            ...
            "lineage": {"inputs": ["purview_5004"], "outputs": ["purview_5001"]},
            "usage": {"queries": 320, "users": 6, "lastAccessed": (now - timedelta(hours=3)).isoformat() + "Z"},
        },
    ]
    ...
    if num_assets and num_assets > len(purview_export):
        ...
        while len(purview_export) < target_count:
            ...
            if is_rot:
                ...
                size_in_bytes = (
                    0 if asset_type == "powerbi_dashboard" else random.randint(1024 * 1024 * 10, 1024 * 1024 * 1024 * 20)
                )
            ...
            if random.random() > 0.4:  # 60% chance of contacts
                contacts["Owner"] = [
                    {"id": f"owner_group_{random.randint(1, 5)}", "info": f"steward.{random.randint(1, 5)}@company.com"}
                ]
            ...
            asset = {
                ...
                "dataQuality": dq,
                "lineage": {"inputs": [], "outputs": []},
                "usage": {"queries": queries, "users": users, "lastAccessed": last_accessed},
            }
